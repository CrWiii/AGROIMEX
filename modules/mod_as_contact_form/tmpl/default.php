...
if(!empty($fields->name)){
	if($params->get('admin_email')) {
		...
		if($params->get('reset_publish')){
			...
			if($labels_pos){
			    $js .= 'if(!$.support.placeholder){$(".mod_as_contact_form *[placeholder]").each(function(n){$(this).parent().find(">.form_placeholder").show()})}';
			}
			$js .= 'return false});';
		}
?>
...
<div id="contact_<?php echo $module->id; ?>">
	<form class="mod_as_contact_form" id="contact-form_<?php echo $module->id; ?>" novalidate>
		...
		<div class="mod_as_contact_form_message" id="message_<?php echo $module->id; ?>">
		...
		<fieldset>
			<div class="row-fluid">
				<?php foreach($fields->type as $key => $type){
					...
					$class = ' class="mod_as_contact_form_'.$type.' hasTooltip"';
				...
				<div class="control-group control-group-button span12">
					<div class="controls">
						<button type="submit" class="btn btn-primary mod_as_contact_form_btn"><?php echo $params->get('bs_name');?></button>
					<?php if($params->get('reset_publish')) { ?>
						<button type="reset" id="clear_<?php echo $module->id; ?>" class="btn btn-primary mod_as_contact_form_btn"><?php echo $params->get('br_name');?></button>
					<?php } ?>
					</div>
				</div>
			</div>
...
<p><?php echo JText::_('MOD_AS_CONTACT_FORM_ENTER_ADMIN_EMAIL'); ?></p>
<?php }
} ?>